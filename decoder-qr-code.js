...
export default class QRCodeDecoder {
    constructor() {
        this.dict = {
            '00': 'Payload Format Indicator',
            '01': 'Point Of Initiation Method',
    ...
    decode(codigo_qr) {
        if (typeof(codigo_qr) !== 'string') {
            ...
            let pacote = codigo.slice(0, 4);
            const id = pacote.slice(0, 2);
            const dataLength = parseInt(pacote.slice(2, 4), 10);
            const dado = codigo.slice(pacote.length, pacote.length + dataLength);
            pacote += dado;

            if (!this.data[this.dict[id]]) {
                this.data[this.dict[id]] = dado;
            }

            const resto = codigo.slice(pacote.length);
            return this.decode(resto);
        } else {
            return this.data;
        }
    }
